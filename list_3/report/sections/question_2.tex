...
\noindent\textbf{Dataset description.}
    ...
    \footnote{
        See the note on data at the end of this report for a more detailed explanation on the features. 
    }, 
...
To find the point of minimum validation error within the hyperparameter space, the \href{https://optuna.org/}{Optuna framework} was used.
...
The low error rates during validation strongly suggest the FNN has compatible complexity with respect to the multiclassification task and hand,
and that the DT is capable of using only the more impactful features - such as UPDRS scores and age - when diagnosing for PD.
%
These models were ran on the test split, resulting in the confusion matrices shown in \cref{fig:confusion_matrices_fnn_dt}
and metrics listed in \cref{tab:results_fnn_dt}.
It is fair to affirm that both models had similarly great performance on classfying both healthiness and PD,
although DT has slightly fewer misclassifications in these cases.

With that said, both models showed very poor results with respect to the prodromal classifications, with 0\% accuracy.
This is, partly, an effect of the optimization objective set during validation:
since models were being selected on overall error rate, and not classwise error rates,
the frequency of classes in the split becomes an influential factor on performance.
In other words, the models tolerate error on prodromal classifications because 
the number of prodromal patients in small when compared to healthy and PD cases.

Future expansions on this study would consider applying or developing a way to make prodromal samples more influential during validation.
One way to do this is making the objective function multivariate, e.g., minimizing not the overall classification error rate,
but a vector of classwise error rates:
since the error rate on classes with a small amount of samples is sensitive to misclassifications,
the proposed objective function would favor it over the error rate of other classes.
...
\begin{table}[!hb]
    \centering
    \begin{threeparttable}
        \caption{Classwise metrics extracted from FNN's and DT's confusion matrices.}
        \label{tab:results_fnn_dt}
        \begin{tabular}{c | ccc ccc ccc}
            \toprule
            \multirow{2}{*}{Model} 
            & \multicolumn{3}{c}{0 (PD)} 
            & \multicolumn{3}{c}{1 (Healthy)} 
            & \multicolumn{3}{c}{2 (Prodromal)} \\
            & Acc.$^*$ & Recall & F1-Score
            & Acc.$^*$ & Recall & F1-Score
            & Acc.$^*$ & Recall & F1-Score \\
            \midrule
...
Optuna's optimization results were not discussed here due to being somewhat off-topic.
For a discussion on Optuna's optimization and its resulting hyperparameters, check \cref{sec:optuna}.