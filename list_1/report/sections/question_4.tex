...
\begin{align}
    ...
    \quad
    g(x) = 
    \log \frac{p(x|C_{+1}) P(C_{+1})}
              {p(x|C_{-1}) P(C_{-1})}
\end{align}

The function $g$, the discriminant of the classifier, is explicitly given by 
%
\begin{align}
    g(x) 
    &=
    \log \frac{p(x|C_{+1}) P(C_{+1})}
              {p(x|C_{-1}) P(C_{-1})} \\
    &=
    \log[p(x|C_{+1})] + \log[P(C_{+1})]
    -\log[p(x|C_{-1})] - \log[P(C_{-1})] \\
    &=
    \begin{cases}
        -\frac{1}{2}\log(2\pi) -\frac{1}{2}(x-2)^2 + \log(0.6) 
        - \log(1/4) - \log(0.4), & |x|\leq 2 \\ 
        \infty, & |x| > 2 
    \end{cases}
...
is a single point $\delta = 2-\sqrt{\log(36/2\pi)}$; 
$x > \delta$ or $x < -2$ are classified as $C_{+1}$, and 
$-2 < x < \delta$ are classified as $C_{-1}$.
...
\begin{align}
    ...
    = 
    0.6 
    \int_{-2}^\delta \frac{1}{\sqrt{2\pi}} e^{-\frac{1}{2}(x-2)^2}\ dx
    +
    0.4 
    \int_\delta^2 \frac{1}{4}\ dx
    =
    0.1880\dots
\end{align}
...
\begin{equation}
    ...
    =
    \begin{cases}
        -\frac{1}{2}\log(2\pi) -\frac{1}{2}(x-2)^2  
        - \log(1/4), & |x|\leq 2 \\ 
        \infty, & |x| > 2 
    \end{cases}
\end{equation}
%
nudges the threshold to $\delta_\text{ML} = 2-\sqrt{\log(16/2\pi)}$ and result in sligthly higher error probability:
%
\begin{align}
